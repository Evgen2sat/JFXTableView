...
import em.libs.jfxtableview.filterFields.filterDouble.JFXFilterFieldDoubleViewImpl;
...
public class JFXDoubleTableColumn<S> extends JFXTableColumn<S, Double> {

    private JFXFilterFieldView filterFieldView;
    private List<FilterTypeModel> filterTypes;

    public JFXDoubleTableColumn() {
        this(null);
    }

    public JFXDoubleTableColumn(String text) {
        super(text, param -> new JFXDefaultTableCell<S, Double>().call((JFXTableColumn<S, Double>)param));
        initTotalRow();
    }
    ...
    public JFXFilterFieldView getFilterFieldView() {
        if (filterFieldView == null) {
            init();
            filterFieldView = new JFXFilterFieldDoubleViewImpl<>(this);
        }
    ...
    @Override
    public Map<ObservableValue<Double>, S> getValues() {
        Map<ObservableValue<Double>, S> values = new HashMap<>();
    ...
    public String getTotalText(List<TotalTypeEnum> selectedTotalTypes) {
        ...
        List<Double> items = ((JFXTableView<S>) getTableView()).getFilteredList().stream().filter(item -> getCellData(item) != null).map(this::getCellData).collect(Collectors.toList());
            ...
            switch (totalType) {
                case AVERAGE:
                    result
                            ...
                            .append(": ")
                            .append(items.stream().mapToDouble(Double::doubleValue).average().orElse(0))
                            .append("; ");
                    break;
                case MINIMUM:
                    result
                ...
                case SUM:
                    result
                            ...
                            .append(": ")
                            .append(items.stream().mapToDouble(Double::doubleValue).sum())
                            .append("; ");
                    break;
            }
        }
