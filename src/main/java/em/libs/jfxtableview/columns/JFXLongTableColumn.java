...
public class JFXLongTableColumn<S> extends JFXTableColumn<S, Long> {

    private JFXFilterFieldView filterFieldView;
    private List<FilterTypeModel> filterTypes;

    public JFXLongTableColumn() {
        this(null);
    }

    public JFXLongTableColumn(String text) {
        super(text, param -> new JFXDefaultTableCell<S, Long>().call((JFXTableColumn<S, Long>)param));
        initTotalRow();
    }
    ...
    public JFXFilterFieldView getFilterFieldView() {
        if (filterFieldView == null) {
            init();
            filterFieldView = new JFXFilterFieldLongViewImpl<>(this);
        }
    ...
    @Override
    public Map<ObservableValue<Long>, S> getValues() {
        Map<ObservableValue<Long>, S> values = new HashMap<>();
